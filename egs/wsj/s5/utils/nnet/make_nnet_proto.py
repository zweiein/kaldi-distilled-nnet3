...
parser.add_option('--softmax-tempurature', dest='softmax_tempurature', 
                   help='Generate <Tempurature>  [default: %default]', 
                   default=1.0, type='float');
parser.add_option('--block-softmax-tempurature', dest='block_softmax_tempurature', 
                   help='Generate <BlockSoftmaxTempurature>  [default: %default]', 
                   default="", type='string');
...
if num_hid_layers == 0 and o.bottleneck_dim != 0:
  ...
  if o.with_softmax:
    ...
    elif o.softmax_tempurature > 1.0 :
      print "<Softmax> <InputDim> %d <OutputDim> %d <Tempurature> %d" % (num_leaves, num_leaves, o.softmax_tempurature)
    elif o.block_softmax_tempurature != "" :
      print "<BlockSoftmax> <InputDim> %d <OutputDim> %d <BlockDims> %s <BlockSoftmaxTempurature> %s" % (num_leaves, num_leaves, o.block_softmax_dims, o.block_softmax_tempurature)
    else:

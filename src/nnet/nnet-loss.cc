...
void MultiTaskLoss::InitFromString(const std::string& s) {
  ...
  //KALDI_ERR << "Size of v : " << v.size() ;

  KALDI_ASSERT((v.size()-1) % 4 == 0); // triplets,
  ...
  for ( ; it != v.end(); ++it) {
    ...
    // --objective-function=multitask,xent,3400,1,1,xent,144,1,20
    // type,
    if (*it == "xent") {
    ...
    ++it;
    // -------2016-03-08-10:10  新增溫度-------
    BaseFloat tempurature;
    if (!ConvertStringToReal(*it, &tempurature)) {
      KALDI_ERR << "Cannot convert 'tempurature' " << *it << " to integer!";
    }
    loss_tempurature_.push_back(tempurature);
    //　^^^^^^ 2016-03-08-10:10 ^^^^^^
  ...
  KALDI_ASSERT(loss_vec_.size() == loss_tempurature_.size());
}
...
std::string MultiTaskLoss::Report() {
  ...
  oss << "Loss (OVERALL), " 
      ...
      << "tempuratures " << loss_tempurature_ << ", "

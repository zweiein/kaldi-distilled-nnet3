...
void Nnet::SetSoftmaxTemurature(BaseFloat st)  {

  for (int32 c=0; c < NumComponents(); c++) {
    //KALDI_ERR << "type c= " << GetComponent(c).GetType() <<" component=" << Component::kTempurature << " kSplice＝ " << Component::kSplice << " kTranform＝ " << Component::kTranform ;

    if (GetComponent(c).GetType() == Component::kTempurature) {
      Tempurature& comp = dynamic_cast<Tempurature&>(GetComponent(c));
      BaseFloat st_old = comp.GetSoftmaxTemurature();
      comp.SetSoftmaxTemurature(st);
      KALDI_LOG << "Setting softmax-temurature in component " << c 
                << " from " << st_old << " to " << st;
    }
  }
}

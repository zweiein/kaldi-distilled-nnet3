...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // -----------------------------
    BaseFloat distilled_tempurature = 1.0; // default = 1
    po.Register("softmax-tempurature", &distilled_tempurature, "softmax-tempurature for distillation");
    // -----------------------------
    ...
    if (no_softmax && (distilled_tempurature != 1.0) ) {
      KALDI_ERR << "Cannot use both --no-softmax=true, distilled-tempuratur>1, use only one of the two!";
    } // end if ( --no-softmax=true & distilled-tempuratur>1 )
    // -----------------------------
    ...
    if ( distilled_tempurature > 1.0 ) {
      nnet_transf.SetSoftmaxTemurature(distilled_tempurature);
      nnet.SetSoftmaxTemurature(distilled_tempurature);
      //KALDI_ERR << " tempurature= " << distilled_tempurature;
    } // if (distilled_tempurature > 1)
